...
export default function ManufacturingTotalBox(props){
    ...
    useEffect(() => {
        ...
        const fetchData2 = async () => {
            try {
                const response2 = await axios.get('/manufacturing/manufacturingInfo.do');
                const nowlength = response2.data.filter((result) => result.mfstate === 0);
                const successlength = response2.data.filter((result) => result.mfstate === 2);
                const faillength = response2.data.filter((result) => result.mfstate === 1);
